...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const categoryEntries = catalogCategories.map((category) => entry(`/products/${category.slug}`, category.id === "cups" ? "2026-09-11" : category.id === "cutlery" ? "2026-09-10" : "2026-09-09"));
  const familyEntries = productFamilies.map((family) => {
    const category = catalogCategories.find((item) => item.id === family.category)!;
    return entry(`/products/${category.slug}/${family.id}`, family.category === "cups" ? "2026-09-11" : family.category === "cutlery" ? "2026-09-10" : "2026-09-09");
  });
