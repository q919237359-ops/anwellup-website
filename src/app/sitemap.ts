...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const categoryEntries = catalogCategories.map((category) => entry(`/products/${category.slug}`, ["cups", "boxes", "cutlery", "tableware", "foil"].includes(category.id) ? "2026-09-11" : "2026-09-09"));
  const familyEntries = productFamilies.map((family) => {
    const category = catalogCategories.find((item) => item.id === family.category)!;
    return entry(`/products/${category.slug}/${family.id}`, ["cups", "boxes", "cutlery", "tableware", "foil"].includes(family.category) ? "2026-09-11" : "2026-09-09");
  });
