...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const staticEntries = staticRoutes.map((route) => entry(route, "2026-09-09"));
  const categoryEntries = catalogCategories.map((category) => entry(`/products/${category.slug}`, category.id === "cutlery" ? "2026-09-10" : "2026-09-09"));
  const familyEntries = productFamilies.map((family) => {
    const category = catalogCategories.find((item) => item.id === family.category)!;
    return entry(`/products/${category.slug}/${family.id}`, family.category === "cutlery" ? "2026-09-10" : "2026-09-09");
  });
