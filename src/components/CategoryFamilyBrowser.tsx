  ...
  return <>
    <section className="category-family-section" aria-labelledby="family-title">
      ...
      {materials.length > 1 && <div className="material-filter category-material-filter" role="group" aria-label="Filter this category by material">
        <span>Material</span><div><button className={material === "all" ? "active" : ""} type="button" aria-pressed={material === "all"} onClick={() => setMaterial("all")}>All</button>{materials.map(item => <button className={material === item ? "active" : ""} type="button" aria-pressed={material === item} onClick={() => setMaterial(item)} key={item}>{item}</button>)}</div>
      </div>}
      <div className="format-list">{filtered.map(family => <article className="format-row" key={family.id}>
        <div className="format-name"><code>{family.sku}</code><h3><button className="family-preview-title" type="button" onClick={() => setSelected(family)} aria-haspopup="dialog">{family.name}</button></h3></div>
        <div className="format-description"><p>{family.summary}</p><span>{family.materials.join(", ")}</span></div>
        <div className="format-actions"><button className="format-action format-preview" type="button" onClick={() => setSelected(family)} aria-haspopup="dialog"><span>{family.variants.length} {family.variants.length === 1 ? "option" : "options"}</span><ArrowRight size={21}/></button><Link href={`/products/${category.slug}/${family.id}/`}>Full details</Link></div>
      </article>)}</div>
    </section>
